...
"""ADDED BY CKS now open the file.cio file in the folder and change the IPRINT: print code (month, day, year) code to 1 """
import subprocess

# open and read the File.cio
with open(os.path.join(txtinout_dir, 'file.cio'), 'r') as fin:   # open file 
    data = fin.read().splitlines(True)

# define the new print code
Stuff1 = data[0:58]
New_print_time =    '               1    | IPRINT: print code (month, day, year)\n'
Stuff2 = data[59:86]

# Stick together the new and old lines 
Stuff1.append(New_print_time)
Stuff1.extend(Stuff2)

# Write it back to File.CIO
with open(os.path.join(txtinout_dir, 'file.cio'), 'w') as fout:  # write new file.cio
    fout.writelines(Stuff1)
	
'''Run SWAT again, this time on a daily resolution'''
os.chdir(txtinout_dir)
print ("____________________________________________\nRunning SWAT AGAIN on daily res...\n")
